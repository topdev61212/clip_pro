...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, fr_pos=10, fr_view=4, skips=[4], class_num=45):
        """
        """
        ...
        self.semantic_linears = nn.ModuleList(
            [nn.Linear(W, W)] + 
            [nn.Linear(W, W) for i in range(self.semantic_linears_num - 1)])
    ...
    def forward(self, xyz, ray_dir):
        ...
        input_pts, input_views = self.pe0(xyz), self.pe1(ray_dir) # B*N_rays*samples 93
        ...
        for i, l in enumerate(self.semantic_linears):
            deep_copy_h = self.semantic_linears[i](deep_copy_h)
            deep_copy_h = F.relu(deep_copy_h)
        semantic = self.semantic_output1(deep_copy_h)
        ...
        return outputs.reshape(B, N_rays, N_samples, 4+self.class_num)

    def forward_semantic(self, xyz):
        input_pts = self.pe0(xyz)
        h = input_pts
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)
            h = F.relu(h)
