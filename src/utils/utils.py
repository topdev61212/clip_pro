...
def validate(out, labels, name_list, out_classes=50):
    device = "cuda:0"
    out = out.to(device)
    ...
    with torch.no_grad():
        ...
        mIoU = np.mean(iou_class[np.where(iou_class != 0)])

        print('Val results: mIoU {:.4f}'.format(mIoU))
        for i in range(out_classes):
            print('class_{} Result: iou {:.4f}.'.format(i, iou_class[i]), name_list[i])
